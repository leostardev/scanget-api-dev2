...
exports.createPresignedURL = function (method, host, path, service, payload, options) {
  ...
  let query = options.query ? querystring.parse(options.query) : {};
  ...
  let canonicalRequest = exports.createCanonicalRequest(method, path, query, options.headers, payload);
  let stringToSign = exports.createStringToSign(options.timestamp, options.region, service, canonicalRequest);
  let signature = exports.createSignature(options.secret, options.timestamp, options.region, service, stringToSign);
